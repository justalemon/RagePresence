$discord_url = "https://github.com/discord/discord-rpc/releases/download/v3.4.0/discord-rpc-win.zip"
$discord_zip = "sdk\\discord.zip"
$discord_out = "sdk\\discord"
...
if (!(Test-Path $discord_out))
{
    # Create the directory
    New-Item -Path "sdk" -Name "discord" -ItemType "directory"
    
    # If the SDK ZIP doesn't exists, download it
    if (!(Test-Path $discord_zip))
    {
        Invoke-WebRequest -Uri $discord_url -OutFile $discord_zip
    }
    
    # Extract the files
    Expand-Archive -Path $discord_zip -DestinationPath $discord_out
    
    # Once everything is done, do some cleanup
    Remove-Item $discord_zip -ErrorAction Ignore
}
